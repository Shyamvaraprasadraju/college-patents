...
const FileField = ({ label, name, fileState, accept = ".pdf", required = false, error, onChange, onRemove }) => (
    <div className="space-y-1.5">
        <label className="text-sm font-medium text-slate-700 flex items-center gap-1.5">
        ...
        <div className="relative group">
            ...
            <label
                htmlFor={name}
                className={`w-full flex items-center justify-between px-4 py-2.5 rounded-lg border border-dashed cursor-pointer transition-all ${
                    error ? "border-red-300 bg-red-50/10 text-red-500" :
                    fileState
...
const UploadForm = ({ onSuccess, onFormChange }) => {
    ...
    const validateForm = () => {
        ...
        if (!files.document) newErrors.document = "Proof of Publication is required";
